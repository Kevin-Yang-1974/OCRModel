...
12. BSCC 定时监控实现：每次提交后必须核验自动化为 `ACTIVE`、绑定当前任务线程、监控 prompt 指向正确的 job/run ID，且频率与任务阶段一致；用户要求“定时查看”时，每次触发须给出简短可见状态，不得仅静默执行。任务状态发生阶段切换、完成、失败或异常时须立即回报；任务完成并完成结果核验后删除该任务的旧监控，避免 stale automation。
