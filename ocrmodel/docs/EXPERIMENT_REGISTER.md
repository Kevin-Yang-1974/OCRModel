...
# 2026-09-22 行级 patch mask v2：五卡 DDP（正式任务完成）

`glmocr_line_mask_v2_smoke_20260922_1940`（10/5工程验证）及正式`glmocr_line_mask_v2_full_20260922_1940`（完整train2159/validation149，seed42，8epochs/3456更新），冻结checkpoint-3000的decoder LoRA、只训确定性行patch head，五卡真DDP/global batch5。最终epoch8 step3456 validation149 CER=`0.12846305276804149`，I/D/S=`796/531/4024`，EOS149/149、触顶0、循环0，`acceptance.passed=true`；对无路由baseline CER=`0.16992365679166466`下降约24.4%。test未读取、不参与选点；未做VAE/MLP/loss单变量消融。完整配置、loss、源码/manifest指纹、产物与限制见[实验日志](实验日志/GLMOCR/架构收益对照/GLMOCR-line-mask-ddp-v2-20260922.md)及[方案](LINE_MASK_DDP_V2.md)。

追加：初始smoke与r2分别因CUPTI库、系统盘临时目录失败；r3普通smoke完成；r4闭环KV因缺next_sequence_length导致OOM，修复后r5完整闭环smoke完成（仅5页，CER0.06556不具验收资格）。所有失败/完成产物保留，详细配置与结果见日志。正式`glmocr_line_mask_v2_full_20260922_1940`已于19:48启动，19:49确认五rank缓存进展；head501892参数，源码tar838c06d...ff4c26，五卡DDP/全2159训练/149验证、未读test。heartbeat mask-ddp初始5分钟，两次健康后改1小时；目前没有正式149页成绩。

追加健康检查2（2026-09-23）：正式run仍在epoch7 routed-cache，五个torchrun worker存活，cache-routed-epoch7约2101/2159文件且rank progress持续更新时间；step2592 metrics finite，错误扫描为空。status旧时间戳与cache阶段相符，不能单独判断停滞；以进程、文件更新时间、progress、log联合核验。heartbeat已按规则降至每小时；test_manifest_read=false、test_used_for_selection=false。

追加（19:53健康检查2）：tmux在、status running/cache、无launcher_status即运行中、五rank progress持续推进（index 455/461/462/463/469，对应每rank约432页分片的尾部）、cache-unbiased 485文件/4.7GiB；异常扫描（NaN/Inf/OOM/Traceback/NCCL/异常退出）为空。两次健康检查通过，监控降为每1小时。

追加（20:18–20:19阶段变化）：缓存完成2159文件/20GiB（五rank index至2155–2158）→ `baseline_validation` → 20:19写字串 `baseline/summary.json`。**无路由baseline 149页**：CER=0.16992365679166466、reference chars41654、I/D/S=1948/1082/4048、exact0、EOS148/触顶1/循环1（loop_rate0.0067）、无删页与budget变化，coverage校验通过。随后进入training/epoch1，`metrics-rank{0..4}.jsonl`已在生成（至step7），五rank同step grad_norm一致（step6=8.672424316453934），loss有限；早期`iou≈0`、`mass_ratio`偏大、`location≈log(patch数)`属随机head初期状态，不作定位结论。全文件风险扫描为空。本地归档`baseline-summary.json`。仍未读test，无epoch2选点结果。

监控更正：Claude Code CLI无桌面Codex式持久heartbeat，仅会话内定时任务（CronCreate `5e39ac7e`，每小时），会话结束即失效，无`automation_update`；已同步写入实验日志。

追加（epoch2选点，21:15核验）：step864/`epoch-2.pt`，完整149页、reference chars41654，**CER=0.1353291400585778**、I/D/S=868/786/3983、exact0、EOS149/触顶0/循环0、无删页与budget变化、coverage通过、`validation_sha256`与protocol一致。对不路由baseline(0.16992365679166466)为−0.0346（−20.4%），字面达CER<=0.14。**限制**：中间epoch2、epoch4/8未完，最终按CER最小重选；149页为development validation非locked test；单臂无等预算消融，不可单独归因某loss项；test未读取。产物已归档`D:/yangky/glm-ocr-assets/line-mask-ddp-20260922/`。

追加（epoch4选点，21:48写出/22:18核验）：step1728/`epoch-4.pt`，149页、chars41654，**CER=0.1341767897440822**、I/D/S=889/666/4034、EOS149/触顶0/循环0、`validation_sha256`一致、`eligible/passed=true`、test未读取。对比：baseline0.16992365679166466、epoch2 0.1353291400585778；对baseline−0.0357（−21.0%）。**如实限制**：epoch2→4仅−0.00115、两epoch均贴近0.14上沿、substitution几乎未降(4034 vs 4048)而降幅主要来自I/D；同源GT整行参照0.12469仍优于本预测mask0.13418（优于GT窗口0.13694）。当前选中点epoch4，最终以epoch8选点为准。此时status=running/epoch6/step2510（五rank metrics2506行、grad_norm一致、风险扫描空），epoch1–5 checkpoint齐全。预告：epoch6后进入epoch7前`routed_cache`闭环刷新（纯缓存无metrics，约20分钟，不算停滞）。

追加（**正式run完成与最终验收**，2026-09-23 02:01完成/02:18核验）：status=complete、`best_validation_cer=0.12846305276804149`、tmux正常退出（五卡0MiB/0%）、无launcher_status即无异常退出；epoch-1..8.pt齐全，五rank metrics各3456行覆盖step1–3456/epoch1–8，风险扫描空。**选点epoch8/step3456**：149页、reference chars41654、**CER=0.12846305276804149**、I/D/S=**796/531/4024**、EOS149/触顶0/循环0/exact0。本地用项目自身`levenshtein_alignment`重算prediction逐位复现同值；149条page_id唯一无重无漏（30/30/30/30/29）；epoch-8.pt 29 tensor/501892参数/all_finite；protocol、selection、磁盘validation manifest三处sha256均为`36ec8458...a348`；test未读取、不参与选点。相对baseline0.16992365679166466为−0.04146（−24.4%），CER≤0.14，脚本`eligible/passed=true` → **验收通过，带保留**：development validation非locked test；单臂无等预算消融不可归因组件；**substitution几乎未改善(4048→4024)，降幅全来自I/D(1948→796、1082→531)**；GT整行参照0.12469仍优于0.12846（已优于GT窗口0.13694）。训练侧末100步：loss1.5327/iou0.6325/mass_ratio2.5306/location4.3116。产物已归档`D:/yangky/glm-ocr-assets/line-mask-ddp-20260922/`（selection-final-epoch8.json、status-final.json、validation-epoch8/、metrics-rank0.jsonl）。下一步建议（未执行）：等预算对照隔离闭环刷新与head贡献、补真实训练字符频次以启用低频诊断、如需locked test须先固定selection。监控已停用。

追加（**补真实训练字符频次与频率诊断**，2026-09-23，只读离线）：原设计五处 `aggregate_ocr_metrics` 调用传空 `Counter()`（含本run `train_line_mask_ddp.py`），致summary的 `low_frequency_k*`/`r2_k*` 全为0/null——属记录缺陷。用train manifest（2159页、train_character_counts实为722207字/6072类）对同一validation149页重算（未重推理、未读test、未参与选点）：epoch8 **k≤1 recall0.6094(64字)/k≤3 0.5556(153字)/k≤5 0.5981(209字)/训练未见0.6341(82字)**，四类合计<1%且baseline→epoch8几乎不变（k≤3 0.5621→0.5556），而高频字0.8787→0.8926——**本轮改进来自高频字，稀有字非瓶颈**。替换误差为**异体/古字→通行正体**（`増→增`524、`縁→緣`178、`䖏→處`99、`閒→間`91、`爲→為`77、`𫝹→念`73…），epoch8 top20混淆对占全部编辑40.7%/替换54.2%；等长页面上落在训练未见字的替换仅6个（baseline2个），故属词表/字形归一化而非定位失败；仅扣除top10混淆对的上限式假设下CER0.12846→0.0830（**非可达成绩**）。baseline最差长页 `mthv2_tkh_0001_021_28_11` 由edits1008/触顶1536 token降至epoch8 edits66/424 token；epoch8全部149页触顶0，编辑仍长尾（top10页占19.6%、top30占41.6%）。代码：`train_line_mask_ddp.py`增 `train_character_counts()` 并在evaluate传入；locked test evaluator/merger/launcher增可选 `--train-manifest`；新增只读工具 `ocrmodel/tools/evaluation/diagnose_line_mask_frequency.py`；产物 `frequency-diagnosis.json` 已归档。既有summary不回写。

追加（loss配比诊断，step1157）：名义BCE35.1%/Dice35.1%，但实际加权贡献早期location37%→晚期**location50.4%、dice35.7%、bce7.4%**，空间项location被BCE/Dice稀释。IoU实为单调上升（step1010.066→step2010.373→step11010.540，近300步均值0.499/max0.850）；dice loss0.96→0.576，其口径为逐token平均含空/EOS行，与`mass_ratio`正相关(corr0.59，面积偏大)。属设计已披露权衡，非故障；如需更快空间收敛的候选旋钮（升location权重或行内归一化BCE/Dice）本轮不自动执行。

正式完成追加（2026-09-23）：epoch8 selection-locked validation149通过`CER<=0.14`，结果`0.12846305276804149`；训练/验证/测试边界、checkpoint finite、coverage、EOS/触顶/循环及本地归档见实验日志。正式结果核验后heartbeat已停用。

## 2026-09-23 行级 mask selection-locked full MTHv2 test
