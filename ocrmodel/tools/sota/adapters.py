...
def _ensure_torchvision_nms_stub(torch_module: Any) -> None:
    """Allow the base environment's image processor to import without compiled ops."""
    global _TORCHVISION_NMS_LIB
    try:
        _TORCHVISION_NMS_LIB = torch_module.library.Library("torchvision", "DEF")
    ...
    def load(self) -> None:
        ...
        try:
            if self.spec.adapter == "paddleocr_vl":
                # Prefer the upstream native implementation when the installed
                # Transformers version already ships PaddleOCR-VL.  The
                # checkpoint's remote code targets an older 5.x API and is
                # kept only as a fallback.
                try:
                    from transformers import PaddleOCRVLForConditionalGeneration as _native_loader
                except Exception:
                    _native_loader = None
                if _native_loader is not None:
                    try:
                        _native_kwargs: dict[str, Any] = {"trust_remote_code": False}
                        import transformers as _tf_mod
                        _dtype_key = "dtype" if int(_tf_mod.__version__.split(".")[0]) >= 5 else "torch_dtype"
                        if self.dtype == "bf16" and torch.cuda.is_available():
                            _native_kwargs[_dtype_key] = torch.bfloat16
                        elif self.dtype == "fp16" and torch.cuda.is_available():
                            _native_kwargs[_dtype_key] = torch.float16
                        self.processor = AutoProcessor.from_pretrained(str(self.model_root), trust_remote_code=False)
                        self.model = _native_loader.from_pretrained(str(self.model_root), **_native_kwargs)
                        if self.device != "cpu" and hasattr(self.model, "to"):
                            self.model.to(self.device)
                        self.model.eval()
                        self._native_paddle = True
                        self.runtime.update(self.parameter_stats())
                        self.runtime["provider"] = "native_transformers_paddleocr_vl"
                        return
                    except Exception as native_exc:  # fall through to official remote code
                        self.runtime["native_paddle_error"] = f"{type(native_exc).__name__}: {native_exc}"[:1000]
                self._native_paddle = False
            ...
            _dtype_key = "dtype" if int(_transformers.__version__.split(".")[0]) >= 5 else "torch_dtype"
            if self.dtype == "bf16" and torch.cuda.is_available():
                kwargs[_dtype_key] = torch.bfloat16
            elif self.dtype == "fp16" and torch.cuda.is_available():
                kwargs[_dtype_key] = torch.float16
            if self.spec.adapter == "paddleocr_vl":
    ...
    def predict(self, image_path: Path, prompt: str) -> tuple[Any, str, dict[str, Any]]:
        ...
        try:
            ...
            with torch.inference_mode():
                ...
                if (self.spec.adapter == "paddleocr_vl"
                        and not getattr(self, "_native_paddle", False)
                        and inputs.get("input_ids") is not None):
...
class OpenDocAdapter(BaseAdapter):
    ...
    def load(self) -> None:
        if os.environ.get("OPENOCR_ONNX", "0") == "1":
            if not self.model_root.exists():
                # The ONNX path uses the pre-staged assets under OPENOCR_ONNX_CACHE,
                # not the Hugging Face unirec checkpoint; make the directory so the
                # adapter contract stays uniform.
                self.model_root.mkdir(parents=True, exist_ok=True)
        elif not self.model_root.exists():
            raise AdapterError(f"deployed_model_missing: {self.model_root}")
        if os.environ.get("OPENOCR_ONNX", "0") == "1":
            try:
                ...
                for candidate in configured_openocr.split(os.pathsep):
                    root_candidate = Path(candidate) / "openocr"
                    if root_candidate.is_dir():
                        openocr_root = root_candidate
                        # OpenOCR's internal absolute ``tools.*`` imports are
                        # resolved by extending this project's ``tools``
                        # package path; this avoids importing the heavy
                        # ``openocr/__init__.py``.
                        if str(candidate) not in sys.path:
                            sys.path.insert(0, str(candidate))
                        project_tools = sys.modules.get("tools")
                        if project_tools is not None and hasattr(project_tools, "__path__"):
                            tools_path = str(root_candidate / "tools")
                            if tools_path not in project_tools.__path__:
                                project_tools.__path__.append(tools_path)
                        break
                if openocr_root is None:
                    raise AdapterError("official_openocr_root_missing: OpenOCR package not found under OPENOCR_PYTHONPATH")
                import importlib.util
                module_spec = importlib.util.spec_from_file_location(
                    "_openocr_infer_doc_onnx", openocr_root / "tools" / "infer_doc_onnx.py"
                )
                if module_spec is None or module_spec.loader is None:
                    raise AdapterError("official_opendoc_module_import_failed: infer_doc_onnx.py")
                module = importlib.util.module_from_spec(module_spec)
                sys.modules[module_spec.name] = module
                module_spec.loader.exec_module(module)
                OpenDocONNX = module.OpenDocONNX
...
class MinerUAdapter(BaseAdapter):
    """Official ``mineru-vl-utils`` transformers backend for MinerU2.5-Pro.

    MinerU2.5-Pro is a Qwen2-VL based 1.2B document parser.  The official
    ``MinerUClient.two_step_extract`` performs layout detection followed by
    content recognition and returns an ordered list of content blocks whose
    ``content`` field carries the recognized text/HTML/LaTeX.
    """
    ...
    def load(self) -> None:
        ...
        try:
            import torch
            _ensure_torchvision_nms_stub(torch)
            from transformers import AutoProcessor, Qwen2VLForConditionalGeneration
            from mineru_vl_utils import MinerUClient
        except Exception as exc:  # pragma: no cover - server dependency
            raise AdapterError(f"official_mineru_dependency_missing: {exc}") from exc
        try:
            kwargs: dict[str, Any] = {"trust_remote_code": True}
            if self.dtype == "bf16" and torch.cuda.is_available():
            ...
            self.model = Qwen2VLForConditionalGeneration.from_pretrained(str(self.model_root), **kwargs)
            self.processor = AutoProcessor.from_pretrained(
                str(self.model_root), trust_remote_code=True, use_fast=False
            )
            if self.device != "cpu" and hasattr(self.model, "to"):
                self.model.to(self.device)
            self.model.eval()
            ...
            for sampling_params in self._client.sampling_params.values():
                sampling_params.max_new_tokens = self.max_output_tokens
            self.runtime.update(self.parameter_stats())
            self.runtime["provider"] = "official_mineru_vl_utils_transformers"
            self.runtime["mineru_sampling_max_new_tokens"] = self.max_output_tokens
        except Exception as exc:  # pragma: no cover - provider-specific server dependency
            details = traceback.format_exc()[-2000:]
            raise AdapterError(f"official_mineru_load_failed: {type(exc).__name__}: {exc}\n{details}") from exc
    ...
    def predict(self, image_path: Path, prompt: str) -> tuple[Any, str, dict[str, Any]]:
        ...
        try:
            ...
            blocks = self._client.two_step_extract(Image.open(image_path).convert("RGB"))
            elapsed = time.perf_counter() - started
            raw = [dict(block) for block in blocks]
            text = "\n".join(
                str(block.get("content")) for block in raw if block.get("content")
            )
            stats = {
                "latency_seconds": elapsed,
                "pages_per_second": 1.0 / elapsed if elapsed else None,
            ...
            if torch.cuda.is_available():
                stats["peak_memory_mib"] = torch.cuda.max_memory_allocated() / (1024 * 1024)
            return raw, normalize_text(text), stats
        except Exception as exc:
            details = traceback.format_exc()[-2000:]
            raise AdapterError(f"official_mineru_inference_failed: {type(exc).__name__}: {exc}\n{details}") from exc
...
def build_adapter(
    ...
    *,
    ...
    if spec.adapter in {"paddleocr_vl", "glm_ocr"}:
        return TransformersImageTextAdapter(
            spec,
            model_root,
            device=device,
            dtype=dtype,
