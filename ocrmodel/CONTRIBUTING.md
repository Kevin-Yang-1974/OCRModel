...
3. 当前论文方法是单个 GOT2 内部的 LAVP，工程子模块为 causal PVLD 与 `visual_value_layout_routing`。Fixed-Slot VLQA/VQLCA 是旧方案，只保留为 baseline 和兼容路径。双 GOT2 两阶段系统、AnandaSky line-level 模型和 AncientDoc 旧 split 均为独立对照或兼容诊断。
