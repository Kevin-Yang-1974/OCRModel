...
def eval_model(args):
    ...
    if args.render:
        ...
        left_num = outputs.count(r'\left')

        if right_num != left_num:
            outputs = outputs.replace(r'\left(', '(').replace('\\right)', ')').replace(r'\left[', '[').replace('\\right]', ']').replace(r'\left{', '{').replace('\\right}', '}').replace(r'\left|', '|').replace('\\right|', '|').replace(r'\left.', '.').replace('\\right.', '.')
