...
当前论文方法统一命名为 **LAVP（Layout-Aware Visual Prompting）**。工程实现是整页 GOT2＋global visual/layout prompts＋causal PVLD 布局重建＋`visual_value_layout_routing`，详见 `../../../docs/GOT2_LAYOUT_ENCODING_PLAN.md`。**`layout_query.py` 中的 Fixed-Slot VLQA 已是旧架构，`layout_writeback_mode=vqlca` 是旧架构的历史写回变体；二者不再属于当前主方案，也不得作为 LAVP 的别名。** 旧配置名、加载路径和脚本继续保留，仅用于旧 checkpoint 兼容、结果溯源和 baseline。当前相关代码包括：
...
- `verify_layout_checkpoint.py`：检查历史 VLQA/projector checkpoint、P1 零门控和最终模型重载；
- `../GOT/model/layout_query.py`：保留 Fixed-Slot VLQA、`layout_value` 和 `vqlca` 的历史实现；LAVP/PVLD 使用 `visual_value_layout_routing`，先用 global prompts 从高分辨率 Vary ViT 特征提取 `layout_evidence=A`，再以两跳 `V_i→A→V_i` 视觉值路由生成等长视觉序列，布局证据不作为 OCR Value；
...
## 历史 A0-A5 Fixed-Slot 消融

`train_GOT_layout.py --ablation_id ... --layout_loss_preset ...` 保留历史统一训练入口。A1 只训练 `mm_projector_vary`；A2 训练 projector 与独立 `GenericVisualTransformerAdapter`；A3/A4 训练 projector 与旧 Fixed-Slot VLQA；A5 保留旧 P1→P2。A3–A5 的名称和结果用于历史复现，不代表当前 LAVP/PVLD 配置。
